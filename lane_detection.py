...
def hough_lines_detection(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    """
    lines = cv2.HoughLinesP(img, rho, theta, threshold, nm.array([]), minLineLength=min_line_len,
...
def weighted_img(img, initial_img, α=0.8, β=1., λ=0.):
    ...
    if len(img.shape) is 2:
        img = nm.dstack((img, nm.zeros_like(img), nm.zeros_like(img)))
...
def compute_lane_from_candidates(line_candidates, img_shape):
    ...
    img_shape: shape of image to which hough transform was applied
    ...
    neg_bias = nm.median([l.bias for l in neg_lines]).astype(int)
    neg_slope = nm.median([l.slope for l in neg_lines])
    x1, y1 = 0, neg_bias
    x2, y2 = -nm.int32(np.round(neg_bias / neg_slope)), 0
    left_lane = Line(x1, y1, x2, y2)

    # interpolate biases and slopes to compute equation of line that approximates right lane
    # median is employed to filter outliers
    lane_right_bias = nm.median([l.bias for l in pos_lines]).astype(int)
    lane_right_slope = nm.median([l.slope for l in pos_lines])
    x1, y1 = 0, lane_right_bias
    x2, y2 = nm.int32(nm.round((img_shape[0] - lane_right_bias) / lane_right_slope)), img_shape[0]
...
def smoothen_over_time(lane_lines):
    ...
    avg_line_lt = nm.zeros((len(lane_lines), 4))
    avg_line_rt = nm.zeros((len(lane_lines), 4))

    for t in range(0, len(lane_lines)):
        avg_line_lt[t] += lane_lines[t][0].get_coords()
        avg_line_rt[t] += lane_lines[t][1].get_coords()

    return Line(*nm.mean(avg_line_lt, axis=0)), Line(*nm.mean(avg_line_rt, axis=0))
...
def color_frame_pipeline(frames, solid_lines=True, temporal_smoothing=True):
    ...
    line_img = nm.zeros(shape=(img_h, img_w))
